...
/**
 * Helper class that simplifies data access via the iBATIS {@link com.ibatis.sqlmap.client.SqlMapClient} API, converting checked
 * SQLExceptions into unchecked DataAccessExceptions, following the {@code org.springframework.dao} exception hierarchy. Uses the same
 * {@link org.springframework.jdbc.support.SQLExceptionTranslator} mechanism as {@link org.springframework.jdbc.core.JdbcTemplate}.
 *
 * <p>
 * The main method of this class executes a callback that implements a data access action. Furthermore, this class provides numerous
 * convenience methods that mirror {@link com.ibatis.sqlmap.client.SqlMapExecutor}'s execution methods.
 *
 * <p>
 * It is generally recommended to use the convenience methods on this template for plain query/insert/update/delete operations. However, for
 * more complex operations like batch updates, a custom SqlMapClientCallback must be implemented, usually as anonymous inner class. For
 * example:
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * The template needs a SqlMapClient to work on, passed in via the "sqlMapClient" property. A Spring context typically uses a
 * {@link SqlMapClientFactoryBean} to build the SqlMapClient. The template an additionally be configured with a DataSource for fetching
 * Connections, although this is not necessary if a DataSource is specified for the SqlMapClient itself (typically through
