...
package com.dev.ck.ackwd.config.springframework.orm.ibatis.support;
...
import com.dev.ck.ackwd.config.springframework.orm.ibatis.SqlMapClientFactoryBean;
import com.ibatis.sqlmap.engine.type.BaseTypeHandler;

/**
 * Abstract base class for iBATIS TypeHandler implementations that map to LOBs. Retrieves the LobHandler to use from SqlMapClientFactoryBean
 * at config time.
 *
 * <p>
 * For writing LOBs, an active Spring transaction synchronization is required, to be able to register a synchronization that closes the
 * LobCreator.
 *
 * <p>
...
public abstract class AbstractLobTypeHandler extends BaseTypeHandler {
	 ...
	 * DataSourceUtils.#CONNECTION_SYNCHRONIZATION_ORDER - 100 to execute LobCreator cleanup before JDBC Connection cleanup, if any.
	 * 
	...
	/**
	 * This implementation delegates to setParameterInternal, passing in a transaction-synchronized LobCreator for the LobHandler of this
	 * type.
	 * 
	 ...
	 *             if thrown by streaming methods
	 */
	protected abstract Object getResultInternal(ResultSet rs, int index, LobHandler lobHandler) throws SQLException, IOException;

	/**
	 * Callback for resource cleanup at the end of a Spring transaction. Invokes LobCreator.close to clean up temporary LOBs that might have
	 * been created.
	 * 
