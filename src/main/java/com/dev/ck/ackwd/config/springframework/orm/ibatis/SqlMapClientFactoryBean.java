...
package com.dev.ck.ackwd.config.springframework.orm.ibatis;
...
/**
 * {@link org.springframework.beans.factory.FactoryBean} that creates an iBATIS {@link com.ibatis.sqlmap.client.SqlMapClient}. This is the
 * usual way to set up a shared iBATIS SqlMapClient in a Spring application context; the SqlMapClient can then be passed to iBATIS-based
 * DAOs via dependency injection.
 *
 * <p>
 * Either {@link org.springframework.jdbc.datasource.DataSourceTransactionManager} or
 * {@link org.springframework.transaction.jta.JtaTransactionManager} can be used for transaction demarcation in combination with a
 * SqlMapClient, with JTA only necessary for transactions which span multiple databases.
 *
 * <p>
 * Allows for specifying a DataSource at the SqlMapClient level. This is preferable to per-DAO DataSource references, as it allows for lazy
 * loading and avoids repeated DataSource references in every DAO.
 *
 * <p>
 * <b>Note:</b> As of Spring 2.5.5, this class (finally) requires iBATIS 2.3 or higher. The new "mappingLocations" feature requires iBATIS
 * 2.3.2.
 *
 ...
 * @deprecated as of Spring 3.2, in favor of the native Spring support in the Mybatis follow-up project (http://code.google.com/p/mybatis/)
 */
@Deprecated
...
public class SqlMapClientFactoryBean implements FactoryBean<SqlMapClient>, InitializingBean {
	...
	/**
	 * Return the LobHandler for the currently configured iBATIS SqlMapClient, to be used by TypeHandler implementations like
	 * ClobStringTypeHandler.
	 * <p>
	 * This instance will be set before initialization of the corresponding SqlMapClient, and reset immediately afterwards. It is thus only
	 ...
	 * <p>
	 * This is an alternative to specifying "&lt;sqlMap&gt;" entries in a sql-map-client config file. This property being based on Spring's
	 * resource abstraction also allows for specifying resource patterns here: e.g. "/myApp/*-map.xml".
	 * <p>
	...
	/**
	 * Set optional properties to be passed into the SqlMapClientBuilder, as alternative to a {@code &lt;properties&gt;} tag in the
	 * sql-map-config.xml file. Will be used to resolve placeholders in the config file.
	 * 
	 * @see #setConfigLocation
	 ...
	 * <p>
	 * If specified, this will override corresponding settings in the SqlMapClient properties. Usually, you will specify DataSource and
	 * transaction configuration <i>either</i> here <i>or</i> in SqlMapClient properties.
	 * <p>
	 * Specifying a DataSource for the SqlMapClient rather than for each individual DAO allows for lazy loading, for example when using
	 * PaginatedList results.
	 * <p>
	 * With a DataSource passed in here, you don't need to specify one for each DAO. Passing the SqlMapClient to the DAOs is enough, as it
	 * already carries a DataSource. Thus, it's recommended to specify the DataSource at this central location only.
	 * <p>
	...
	/**
	 * Set whether to use a transaction-aware DataSource for the SqlMapClient, i.e. whether to automatically wrap the passed-in DataSource
	 * with Spring's TransactionAwareDataSourceProxy.
	 * <p>
	 * Default is "true": When the SqlMapClient performs direct database operations outside of Spring's SqlMapClientTemplate (for example,
	 * lazy loading or direct SqlMapClient access), it will still participate in active Spring-managed transactions.
	 * <p>
	 * As a further effect, using a transaction-aware DataSource will apply remaining transaction timeouts to all created JDBC Statements.
	 * This means that all operations performed by the SqlMapClient will automatically participate in Spring-managed transaction timeouts.
	 * <p>
	 * Turn this flag off to get raw DataSource handling, without Spring transaction checks. Operations on Spring's SqlMapClientTemplate
	 * will still detect Spring-managed transactions, but lazy loading or direct SqlMapClient access won't.
	 * 
	 * @see #setDataSource
	 ...
	 * <p>
	 * Will only get applied when using a Spring-managed DataSource. An instance of this class will get populated with the given DataSource
	 * and initialized with the given properties.
	 * <p>
	 * The default ExternalTransactionConfig is appropriate if there is external transaction management that the SqlMapClient should
	 * participate in: be it Spring transaction management, EJB CMT or plain JTA. This should be the typical scenario. If there is no active
	 * transaction, SqlMapClient operations will execute SQL statements non-transactionally.
	 * <p>
	 * JdbcTransactionConfig or JtaTransactionConfig is only necessary when using the iBATIS SqlMapTransactionManager API instead of
	 * external transactions. If there is no explicit transaction, SqlMapClient operations will automatically start a transaction for their
	 * own scope (in contrast to the external transaction mode, see above).
	 * <p>
	 * <b>It is strongly recommended to use iBATIS SQL Maps with Spring transaction management (or EJB CMT).</b> In this case, the default
	 * ExternalTransactionConfig is fine. Lazy loading and SQL Maps operations without explicit transaction demarcation will execute
	 * non-transactionally.
	 * <p>
	 * Even with Spring transaction management, it might be desirable to specify JdbcTransactionConfig: This will still participate in
	 * existing Spring-managed transactions, but lazy loading and operations without explicit transaction demaration will execute in their
	 * own auto-started transactions. However, this is usually not necessary.
	 * 
	 * @see #setDataSource
	 * @see #setTransactionConfigProperties
	...
	/**
	 * Set properties to be passed to the TransactionConfig instance used by this SqlMapClient. Supported properties depend on the concrete
	 * TransactionConfig implementation used:
	 * <p>
	 * <ul>
	 * <li><b>ExternalTransactionConfig</b> supports "DefaultAutoCommit" (default: false) and "SetAutoCommitAllowed" (default: true). Note
	 * that Spring uses SetAutoCommitAllowed = false as default, in contrast to the iBATIS default, to always keep the original autoCommit
	 * value as provided by the connection pool.
	 * <li><b>JdbcTransactionConfig</b> does not supported any properties.
	 * <li><b>JtaTransactionConfig</b> supports "UserTransaction" (no default), specifying the JNDI location of the JTA UserTransaction
	 ...
	 * <p>
	 * The default implementation uses the standard iBATIS {@link SqlMapClientBuilder} API to build a SqlMapClient instance based on an
	 * InputStream (if possible, on iBATIS 2.3 and higher) or on a Reader (on iBATIS up to version 2.2).
	 * 
	 ...
	 * <p>
	 * The default implementation casts to ExtendedSqlMapClient, retrieves the maximum number of concurrent transactions from the
	 * SqlMapExecutorDelegate, and sets an iBATIS TransactionManager with the given TransactionConfig.
	 * 
	 * @param sqlMapClient
